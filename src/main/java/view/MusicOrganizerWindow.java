...
public class MusicOrganizerWindow extends Application {
    ...
    private ButtonPaneHBox HButtons;
    private ButtonPaneVBox VButtons;
    ...
    public void start(Stage primaryStage) {
        ...
        try {
            ...
            HButtons = new ButtonPaneHBox(controller, this);
            bord.setTop(HButtons);

            //Create the buttons to the right of the GUI
            VButtons = new ButtonPaneVBox(controller, this);
            bord.setRight(VButtons);

            //Gets the initial albums
            Album flaggedAlbum = controller.getFlaggedsoundclipsAlbum();
            Album greatAlbum = controller.getGreatsoundclipsAlbum();

            //Create the tree in the left of the GUI
            tree = createTreeView();

            //adds the initial albums to the tree view
            tree.getRoot().getChildren().add(new TreeItem<>(flaggedAlbum));
            tree.getRoot().getChildren().add(new TreeItem<>(greatAlbum));
            tree.getRoot().setExpanded(true); // automatically expand the parent node in the tree
        ...
        v.setOnMouseClicked(new EventHandler<MouseEvent>() {
            ...
            public void handle(MouseEvent e) {
                if (e.getClickCount() == 2) {
                    // Displays the content of the double clicked album.
                    soundClipTable.display(getSelectedAlbum());

                    //Hides the album modifying buttons when an search based album is selected
                    if (getSelectedAlbum() == controller.getFlaggedsoundclipsAlbum() ||
                            getSelectedAlbum() == controller.getGreatsoundclipsAlbum()) {
                        hideAlbumModifyingButtons();
                    } else {
                        showAlbumModifyingButtons();
                    }
    ...
    /*
     * Pop up a dialog box prompting the user for rating for the sound clip
     * and returns the rating.
     */
    public String promptForRateSoundClip() {
        TextInputDialog dialog = new TextInputDialog();

        dialog.setTitle("Enter sound clip rating");
        dialog.setHeaderText(null);
        dialog.setContentText("Please enter the rating of the sound clip (0-5)");
        Optional<String> result = dialog.showAndWait();
        if (result.isPresent()) {
            return result.get();
        } else {
    ...
    public void hideAlbumModifyingButtons() {
        HButtons.getRemoveSoundClipsButton().setVisible(false);
        ;
        HButtons.getAddSoundClipsButton().setVisible(false);
        HButtons.getNewAlbumButton().setVisible(false);
        HButtons.getDeleteAlbumButton().setVisible(false);
    }

    public void showAlbumModifyingButtons() {
        HButtons.getRemoveSoundClipsButton().setVisible(true);
        HButtons.getAddSoundClipsButton().setVisible(true);
        HButtons.getNewAlbumButton().setVisible(true);
        HButtons.getDeleteAlbumButton().setVisible(true);
    }
    /**
     * *****************************************************************
    ...
    public void onClipsUpdatedNoAlbumSelected() {
        soundClipTable.display(controller.getRootAlbum());
    }


    // This method updates the visibility of the redo button based on the size of the redo stack
    public void updateRedoButtonVisibility(Stack<Memento> redoStack) {
        if (redoStack.empty()) {
            VButtons.setVisibilityRedoButton(false);
        } else {
            VButtons.setVisibilityRedoButton(true);
        }
    }

    // This method updates the visibility of the undo button based on the size of the undo stack
    public void updateUndoButtonVisibility(Stack<Memento> undoStack) {
        if (undoStack.empty()) {
            VButtons.setVisibilityUndoButton(false);
        } else {
            VButtons.setVisibilityUndoButton(true);
        }
    }
